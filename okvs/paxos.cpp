...
uint64_t OBDTables::getDHBits(osuCrypto::block key){
    auto bits = DH(key);
...
void OBD2Tables::createSets(){
    ...
    first = unordered_set<osuCrypto::block, Hasher>(hashSize*factorSize, Hasher(firstSeed));
    second = unordered_set<osuCrypto::block, Hasher>(hashSize*factorSize, Hasher(secondSeed));
...
vector<uint64_t> OBD2Tables::dec(osuCrypto::block key){
...
vector<uint64_t> OBD2Tables::decOptimized(osuCrypto::block key){
//    auto keyIndices = indices[key];
//    if(keyIndices.size() == 0) {
//        cout<<"first time"<<endl;
    ...
    auto dhBits = getDHBits(key);
    uint8_t* dhBytes = (uint8_t*) (&dhBits);
    for (int j = 0; j < 8; j++) {
...
vector<uint8_t> OBD2Tables::decode(osuCrypto::block key){
    ...
    vector<uint8_t> valBytes(fieldSizeBytes);
...
//    }
    ...
    //simply comment it. there is an issue for solve, will get  *** ERROR *** : solve<RingCategories::GenericTag, Method::SparseElimination> does not exists.
...
void OBD3Tables::createSets(){
    ...
    first = unordered_set<osuCrypto::block, Hasher>(hashSize*factorSize, Hasher(firstSeed));
    second = unordered_set<osuCrypto::block, Hasher>(hashSize*factorSize, Hasher(secondSeed));
    third = unordered_set<osuCrypto::block, Hasher>(hashSize*factorSize, Hasher(thirdSeed));

    // std::cout<<"The minimum inition bucket size: "<<(hashSize*factorSize)<<std::endl;
    
    tableRealSize = first.bucket_count();

    // std::cout<<"The real bucket size: "<<(tableRealSize)<<std::endl;
...
vector<uint64_t> OBD3Tables::dec(osuCrypto::block key){
...
vector<uint64_t> OBD3Tables::decOptimized(osuCrypto::block key){
//    auto keyIndices = indices[key];
//    if(keyIndices.size() == 0) {
//        cout<<"first time"<<endl;
...
vector<uint8_t> OBD3Tables::decode(osuCrypto::block key){
    auto indices = dec(key);
...
//        }
    ...
    // to keep it oblivious
    nonOccupiedBins.resize(tableRealSize*3, 0);
    for(int i=0; i<tableRealSize; i++){
        if(first.bucket_size(i) == 0){
            nonOccupiedBins[i] = 1;
        }
        if(second.bucket_size(i) == 0){
            nonOccupiedBins[tableRealSize+i] = 1;
        }
        if(third.bucket_size(i) == 0){
            nonOccupiedBins[2*tableRealSize+i] = 1;
        }
    }
...
void OBD3Tables::handleQueue(queue<int> &queueMain, unordered_set<osuCrypto::block, Hasher> &main,
...
void OBD3Tables::generateExternalToolValues(){
    ...
    NTL::mat_GF2E matrix;
    NTL::vec_GF2E values;
    matrix.SetDims(matrixSize, 3*matrixSize+gamma);
    values.SetLength(matrixSize);
...
//    cout << "construct time in milliseconds for protocol: " << duration << endl;
    ...
    //move from constructor to enable multiple encoding only by unpeeling
    variables.clear();
    variables.resize(3*tableRealSize + gamma, to_GF2E(0));
    ...
    //***** to keep it oblivious
    GF2X temp;
    for(int i=0; i<nonOccupiedBins.size(); i++){
        if(nonOccupiedBins[i] == 1){
            vector<uint8_t> r;
            r.resize(fieldSizeBytes);
            GF2XFromBytes(temp, (unsigned char *)&r ,fieldSizeBytes);
    ...
    for(int i=0; i<gamma; i++){
        ...
        r.resize(fieldSizeBytes);
        GF2XFromBytes(temp, (unsigned char *)&r ,fieldSizeBytes);
        variables[3*tableRealSize+i] = to_GF2E(temp);
    }
...
//    start = high_resolution_clock::now();
    ...
    NTL::vec_GF2E variablesSlim;
    variablesSlim.SetLength(3*matrixSize + gamma);
    solver(matrix, values, variablesSlim);
...
//    start = high_resolution_clock::now();
    ...
    if(peelingCounter != hashSize){
        for (int i=0; i<gamma; i++){
            int indice = 3*tableRealSize + i;
            variables[indice] = variablesSlim[3*matrixSize + i];
        }
    }
...
//    cout<<"in unpeeling"<<endl;
    ...
    // std::cout<<"peelingCounter before: "<<peelingCounter<<std::endl;

    int original_peelingCount = peelingCounter;
    while (peelingCounter > 0){
//            cout<<"key = "<<key<<endl;
        key = peelingVector[--peelingCounter];
        auto indices = dec(key);
...
//                randomVal = prg.getPRGBytesEX(fieldSizeBytes);
    ...
    // std::cout<<"peelingCounter after: "<<peelingCounter<<std::endl;
    // std::cout<<"variables[0]: "<<variables[0]<<std::endl;
    // std::cout<<"variables.size(): "<<variables.size()<<std::endl;

    //for the next unpeeling next time
    peelingCounter = original_peelingCount;
//    cout<<"peelingCounter = "<<peelingCounter<<endl;
//    cout<<"variables:"<<endl;
//    for (int i=0; i<variables.size(); i++){
//        cout<<"variable["<<i<<"] = "<<variables[i]<<" ";
...
void StarDictionary::setKeysAndVals(vector<osuCrypto::block>& keys, vector<uint8_t>& values){
    ObliviousDictionary::setKeysAndVals(keys, values);
    keysForBins.resize(q, vector<osuCrypto::block>(numItemsForBin));
    int fieldSizeBytes = fieldSize % 8 == 0 ? fieldSize/8 : fieldSize/8 + 1;
    valsForBins.resize(q, vector<uint8_t>(numItemsForBin*fieldSizeBytes));
...
vector<uint64_t> StarDictionary::dec(osuCrypto::block key){
...
vector<uint64_t> StarDictionary::decOptimized(osuCrypto::block key){
//    auto keyIndices = indices[key];
//    if(keyIndices.size() == 0) {
//        cout<<"first time"<<endl;
    int binIndex = hashForBins(key) % q;
...
vector<uint8_t> StarDictionary::decode(osuCrypto::block key){
    int index = hashForBins(key) % q;

    vector<uint8_t> first = bins[index]->decode(key);
    vector<uint8_t> second = bins[center]->decode(key);
...
bool StarDictionary::encode() {
    ...
    if (failureIndex == -1){//all bins have peeled succesfully
        ...
        vector<uint8_t> valsForCenter(numItemsForBin*fieldSizeBytes);
...
bool StarDictionary::checkOutput(osuCrypto::block key, int valIndex){
    bool error = false;

    vector<uint8_t> temp1(fieldSizeBytes, 0);
    vector<uint8_t> temp2(fieldSizeBytes, 0);

    vector<uint8_t> variables = getVariables();
    int index = hashForBins(key) % q;
...
vector<uint8_t> StarDictionary::getVariables()  {
...
//    cout<<"innerSize in getVariables = "<<innerSize/fieldSizeBytes<<endl;
    vector<uint8_t> variables((q+1)*innerSize);
